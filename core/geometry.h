...
template <typename T> struct vec<3,T> {
    ...
    vec<3,T>& operator+=(const vec<3,T> &rhs){ *this = *this+rhs; return *this;}
    vec<3,T>& operator-=(const vec<3,T> &rhs){ *this = *this-rhs; return *this;}
    vec<3,T>& operator*=(const T rhs){ *this = *this*rhs; return *this;}
    // vec<3,T>& operator/=(const float rhs){ *this = this/rhs; return *this;}
	
    T norm() { return std::sqrt(x*x+y*y+z*z); }
    T norm() const { return std::sqrt(x*x+y*y+z*z); }
	T length_squared() { return x*x+y*y+z*z; }
...
public:
	...
	vec<DimRows, T> col(const size_t idx) const { //取idx列的向量
		assert(idx < DimCols);
		vec<DimRows, T> ret;
	...
	void set_col(size_t idx, vec<DimRows, T> v) { //set idx 列的向量
		assert(idx < DimCols);
	...
	mat<DimRows, DimCols, T> adjugate() const { // 伴随矩阵
		mat<DimRows, DimCols, T> ret;
		for (size_t i = DimRows; i--; )
